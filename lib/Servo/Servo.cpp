...
void LegConfig::LegInit(FSUS_Protocol INputPol, uint8_t ServoID, uint8_t ServoID2, uint8_t ServoID3)
{

    this->serial = defaultSerial;       // 串口ID
    this->ServoBaud = defaultServoBaud; // 舵机波特率
    this->ServoID = defaultServoID;     // 舵机ID
    this->ServoID2 = defaultServoID2;   // 舵机ID2
    this->ServoID3 = defaultServoID3;   // 舵机ID3

    FSUS_Protocol defaultPol((this->serial), this->ServoBaud);
    this->protocol = defaultPol;                        // 初始化舵机串口通信协议
